...
from sujsim.core.spells.buff_database.debuffs import CASTING_5_SECOND_RULE
...
DEFAULT_PRIORITY = 10
MANA_REGEN_PRIORITY = DEFAULT_PRIORITY
SPELL_CAST_PRIORITY = 3
HEALTH_GAIN_PRIORITY = 2
MANA_GAIN_PRIORITY = 1
BUFF_PRIORITY = 0


class Event:
    def __init__(self, time: float, source_actor: Actor, target_actor: Actor, priority: int = DEFAULT_PRIORITY):
        self.time = time
    ...
    def __str__(self):
        return '{} - {}'.format(type(self), self.time)
...
class MageCastEvent(Event):
    def __init__(self, time: float, spell: Spell, source_actor: Actor, target_actor: Actor, spell_target: Actor):
        super().__init__(time=time, target_actor=target_actor, source_actor=source_actor, priority=SPELL_CAST_PRIORITY)
    ...
    def execute(self):
        ...
        if self.spell.spell_type == MageSpells.EVOCATION:
            self.source_actor.reset_mana_regen_tick()
    ...
    def execute_spell_cast(self):
        self.source_actor.add_to_queue(GainBuffEvent(buff=CASTING_5_SECOND_RULE,
                                                     source_actor=self.source_actor,
                                                     target_actor=self.source_actor))
        cast_spell = self.source_actor.character.calculate_cast_spell(spell=self.spell, target=self.spell_target)
        ...
        if cast_spell.result == SpellResult.MISS:
            ...
            if cast_spell.magic_school == MagicSchool.FROST and self.source_actor.character.mage_talents.winters_chill:
                if self.source_actor.character.mage_talents.winters_chill == 5 or random.randrange(0,
                                                                                                   5) < self.source_actor.character.mage_talents.winters_chill:
            ...
            if self.source_actor.character.gear_stats.has_trinket(trinket.QUAGMIRRANS_EYE) and not self.source_actor.has_buff(
                    combat_buffs.QUAGMIRRANS_EYE) and random.randrange(0, 20) == 0:
            ...
            if self.source_actor.character.gear_stats.has_trinket(trinket.MARK_OF_DEFIANCE) and not self.source_actor.has_buff(
                    debuffs.MARK_OF_DEFIANCE_COOLDOWN) and random.randrange(0, 100) < 15:
                ...
                self.source_actor.add_to_queue(ManaEvent(target_actor=self.target_actor,
                                                         source_actor=self.target_actor,
                                                         amount=random.randrange(128, 174),
                                                         gain=True),
                                               execute_now=True)

            # 5% proc rate ?
            if self.source_actor.character.gear_stats.has_spellstrike() and random.randrange(0, 20) == 0:
            ...
            if self.source_actor.character.gear_stats.has_ring(ring.WRATH_OF_CENARIUS) and not self.source_actor.has_buff(
                    combat_buffs.SPELL_BLASTING) and random.randrange(0, 20) == 0:
            ...
            if config->mana_etched_4set and random.randrange(0, 50) == 0:
            ...
            # 50% proc rate
            if self.spell_target.has_buff(debuffs.JUDGEMENT_OF_WISDOM) and random.randrange(0, 2) == 1:
                self.source_actor.add_to_queue(ManaEvent(target_actor=self.target_actor,
                                                         source_actor=self.target_actor,
                                                         amount=74,
                                                         gain=True),
            ...
            if cast_spell.result == SpellResult.CRIT:
                ...
                if self.source_actor.character.gear_stats.has_trinket(trinket.SEXTANT_OF_UNSTABLE_CURRENTS) and not self.source_actor.has_buff(
                        combat_buffs.UNSTABLE_CURRENTS) and random.randrange(0, 5) == 0:
                ...
                if self.source_actor.character.gear_stats.has_trinket(trinket.SHIFFARS_NEXUS_HORN) and not self.source_actor.has_buff(
                        combat_buffs.CALL_OF_THE_NEXUS) and random.randrange(0, 5) == 0:
                ...
                # 50% proc rate
                if self.source_actor.character.gear_stats.has_trinket(trinket.ASHTONGUE_TALISMAN_MAGE) and random.randrange(0, 2) == 0:
                ...
                if cast_spell.magic_school == MagicSchool.FIRE and self.source_actor.character.mage_talents.ignite:
                    # addIgnite(spell)
                    pass
                if (cast_spell.magic_school == MagicSchool.FIRE or cast_spell.magic_school == MagicSchool.FROST) and \
    ...
    def _execute_presence_of_mind(self):
        if self.source_actor.has_buff(combat_buffs.PRESENCE_OF_MIND):
            self.source_actor.add_to_queue(LoseBuffEvent(buff=combat_buffs.PRESENCE_OF_MIND,
                                                         target_actor=self.target_actor,
                                                         source_actor=self.target_actor,
                                                         time=0),
                                           execute_now=True)
...
class ManaEvent(Event):
    def __init__(self, source_actor: Actor, target_actor: Actor, amount: int, gain: bool):
        super().__init__(time=0, target_actor=target_actor, source_actor=source_actor, priority=MANA_GAIN_PRIORITY)
    ...
    def execute(self):
        ...
        if self.gain:
            ...
            self.target_actor.mana_given[self.source_actor] += self.amount
...
class HealthEvent(Event):
    def __init__(self, time, source_actor: Actor, target_actor: Actor, cast_spell: CastSpell, gain: bool):
        super().__init__(time=time, target_actor=target_actor, source_actor=source_actor, priority=HEALTH_GAIN_PRIORITY)
        self.gain = gain
    ...
    def execute(self):
        ...
        if self.gain:
            ...
            self.target_actor.healing_given[self.source_actor] += self.cast_spell.damage
        else:
            self.target_actor.character.stats.current_health -= self.cast_spell.damage
            self.target_actor.damage_taken[self.source_actor] += self.cast_spell.damage
...
class ManaRegenEvent(Event):
    def __init__(self, time: float, source_actor: Actor, target_actor: Actor):
        super().__init__(time=time, target_actor=target_actor, source_actor=source_actor, priority=MANA_REGEN_PRIORITY)
...
class GainBuffEvent(Event):
    def __init__(self, buff: Buff, source_actor: Actor, target_actor: Actor):
        super().__init__(time=0, target_actor=target_actor, source_actor=source_actor, priority=BUFF_PRIORITY)
        self.buff = buff
...
class LoseBuffEvent(Event):
    def __init__(self, time: float, buff: Buff, source_actor: Actor, target_actor: Actor):
        super().__init__(time=time, target_actor=target_actor, source_actor=source_actor, priority=BUFF_PRIORITY)
        self.buff = buff
